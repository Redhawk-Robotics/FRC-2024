...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    // Driver_XButton.whileTrue(new InstantCommand(() -> pivot.pivot(1)))
    // .toggleOnFalse(new InstantCommand(() -> pivot.pivot(0)));

    // Driver_BButton.whileTrue(new InstantCommand(() -> pivot.pivot(-1)))
    // .toggleOnFalse(new InstantCommand(() -> pivot.pivot(0)));
    ...
    // new InstantCommand(() -> Pivot.setPivotState(PivotStates.kPivotSubwoofer))));
    ...
    OP_left.whileTrue(new IntakeToShooter(intake, pivot, shooter))
        .whileFalse(new InstantCommand(() -> CommandPreparer.prepareToStopIntakeToPivot()));
    ...
    // ^Center Line
    OP_XButton.whileTrue(shooter.rejectNoteCenterLine().alongWith(intake.enableReverseIntake()))
        .whileFalse(shooter.stopNoteRejection().alongWith(intake.stopIntake()));
  ...
  public Command getAutonomousCommand() {
    // !DONT TOUCH
    var path = new PathPlannerAuto(m_autonChooser.getSelected());
    // System.out.println("got auto!!!");
    // (interrupted) -> {
    // System.out.println("im here!");
    // gyroIO.offsetGyroPosition(180);}
    return new FunctionalCommand(
        path::initialize,
        path::execute,
        new Consumer<Boolean>() {
          public void accept(Boolean end) {
            // System.out.println("im here!");
            gyroIO.offsetGyroPosition(180);
          }
        },
  ...
  public void setupNamedCommands() {
    ...
    NamedCommands.registerCommand(
        ...
        new SequentialCommandGroup(
            ...
            new PivotToShoot(
                pivot, PivotStates.kPivotNoteSides, ShooterWheelStates.kShooterThreeQuarterShot)));
    ...
    NamedCommands.registerCommand(
        "Shoot CS",
        new ShootNote(
            shooter, pivot, PivotStates.kPivotHome, ShooterWheelStates.kShooterThreeQuarterShot));
  }

  public void configrueAutos() {
    m_autonChooser.addOption("3P MID AMPSIDE", Settings.AutoPaths.K3P_MID_AMPSIDE);
    m_autonChooser.addOption("ONE", Settings.AutoPaths.kONE);
    m_autonChooser.addOption("3P MID SOURCESIDE", Settings.AutoPaths.k3P_MID_SOURCESIDE);
    m_autonChooser.addOption("3P AMPSIDE", Settings.AutoPaths.k3P_AMPSIDE);
    m_autonChooser.addOption("3P SOURCE", Settings.AutoPaths.k3P_SOURCESIDE);
    m_autonChooser.addOption("3P AMP PICKER", Settings.AutoPaths.k3P_AMP_PICKER);
