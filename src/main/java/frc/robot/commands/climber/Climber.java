...
public class Climber extends Command {
  ...
  public void execute() {
    double leftYaxis =
        MathUtil.applyDeadband(leftPower.getAsDouble(), SwerveConfig.stickDeadband) / 2;
    double rightYaxis =
        MathUtil.applyDeadband(rightPower.getAsDouble(), SwerveConfig.stickDeadband) / 2;

    climber.climberApplySpeed(
        MathUtil.clamp(leftYaxis, -.5, .5), MathUtil.clamp(rightYaxis, -.5, .5));
  }
